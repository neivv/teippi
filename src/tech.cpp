...
void Unit::Order_Feedback(ProgressUnitResults *results)
{
    ...
    if (target && target->Type().Flags() & UnitFlags::Spellcaster && ~target->flags & UnitStatus::Hallucination)
    {
        ...
        if (SpellOrder(this, TechId::Feedback, GetSightRange(true) * 32, &energy_cost, String::Error_MustTargetSpellcaster))
        {
...
void Unit::SpawnBroodlings(const Point &pos_)
{
    ...
    {
        if (!DoesFitHere(UnitId::Broodling.Raw(), pos.x, pos.y))
        {
        ...
        ClipPointInBoundsForUnit(UnitId::Broodling.Raw(), pos_arr);

        Unit *broodling = CreateUnit(UnitId::Broodling.Raw(), pos_arr[0], pos_arr[1], player);
        if (!broodling)
        {
...
void DarkSwarm(int player, const Point &position)
{
    ...
    ClipPointInBoundsForUnit(UnitId::DarkSwarm.Raw(), pos);
    Unit *swarm = CreateUnit(UnitId::DarkSwarm.Raw(), pos[0], pos[1], NeutralPlayer);
    if (!swarm)
    {
        ShowLastError(player);
        return;
...
{
    uint16_t pos[2] = { position.x, position.y };
    ClipPointInBoundsForUnit(UnitId::DisruptionWeb.Raw(), pos);
    Unit *dweb = CreateUnit(UnitId::DisruptionWeb.Raw(), pos[0], pos[1], NeutralPlayer);
    if (dweb == nullptr)
    {
        ShowLastError(player);
        return;
...
{
    if (!target->IsInvincible() && CanTargetSpellOnUnit(TechId::Consume, target, player))
    {
    ...
    if (order_state == 0)
    {
        ...
        else
        {
            if (player == *bw::local_player_id && *bw::select_command_user == *bw::local_unique_player_id)
                PrintInfoMessage((*bw::stat_txt_tbl)->GetTblString(String::NotEnoughEnergy + Type().Race()));
            OrderDone();
        }
    }
    else if (!order_timer)
...
{
    uint16_t energy_cost;
    if (SpellOrder(this, TechId::Hallucination.Raw(), GetSightRange(true) * 32, &energy_cost, String::Error_HallucinationTarget) == 0)
        return;
    ...
    AddOverlayHighest(target->GetTurret()->sprite.get(), ImageId::HallucinationSmoke.Raw(), 0, 0, 0);
    OrderDone();
}
...
void Unit::Order_MindControl(ProgressUnitResults *results)
{
    ...
    uint16_t energy_cost;
    if (SpellOrder(this, TechId::MindControl.Raw(), GetSightRange(true) * 32, &energy_cost, String::Error_MindControlTarget) == 0)
        return;
